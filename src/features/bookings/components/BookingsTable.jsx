        ...
        <div className="bg-gray-50 border-b border-gray-200">
          <div className="grid grid-cols-6 gap-4 px-6 py-3">
            {TABLE_HEADERS.map((header) => (
              <div key={header.key} className="h-5 bg-gray-200 rounded animate-pulse"></div>
            ))}
          </div>
        </div>
        ...
        <div className="divide-y divide-gray-200">
          {[...Array(5)].map((_, index) => (
            <div key={index} className="grid grid-cols-6 gap-4 px-6 py-4">
              {TABLE_HEADERS.map((header) => (
                <div key={header.key} className="h-4 bg-gray-200 rounded animate-pulse"></div>
              ))}
            </div>
          ))}
        </div>
        ...
        <div className="p-12 text-center">
          <div className="mx-auto h-12 w-12 text-gray-400 mb-4">
            <svg fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M8 7V3a2 2 0 012-2h4a2 2 0 012 2v4m-6 0V7a2 2 0 012-2h4a2 2 0 012 2v0M8 7v8a2 2 0 002 2h4a2 2 0 002-2V7M8 7H6a2 2 0 00-2 2v8a2 2 0 002 2h2m0 0h8m-8 0V9a2 2 0 012-2h4a2 2 0 012 2v8a2 2 0 01-2 2H10z" />
            </svg>
          </div>
          <h3 className="text-lg font-medium text-gray-900 mb-2">No hay reservas</h3>
          <p className="text-gray-500 mb-4">No se encontraron reservas que coincidan con los filtros aplicados.</p>
    ...
    <div className={`bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden ${className}`}>
      ...
      <div className="bg-gray-50 px-6 py-3 border-t border-gray-200">
        <div className="flex items-center justify-between text-sm text-gray-600">
          <div>
            Mostrando {sortedBookings.length} reserva{sortedBookings.length !== 1 ? 's' : ''}
          </div>
          <div className="flex items-center space-x-4">
            <span>Ordenado por {SORT_FIELDS[sortBy]?.label.toLowerCase()}</span>
            <button
